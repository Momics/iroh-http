...
//! ([`iroh_http_core::serve_with_events_inner`]) is callable from a
...
use bytes::Bytes;
use iroh_http_core::{
    fetch, serve_with_events_inner, Body, RemoteNodeId, ServeOptions,
};
use tower::Service;
...
async fn pure_rust_serve_round_trips_with_peer_id_extension() {
    ...
    let _handle = serve_with_events_inner(
        server_ep.clone(),
