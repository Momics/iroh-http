/**
 * iroh-http session tests — Deno
 *
 * Tests node.connect(), node.sessions(), IrohSession properties,
 * bidirectional streams, unidirectional streams, and datagrams.
 *
 * All tests use server.sessions() as the acceptor — the server starts
 * accepting sessions in the background, then the client connects.
 *
 * Usage:
 *   deno run -A tests/sessions/test-deno.ts
 */
