...
export type RawServeFn = (endpointHandle: number, options: {
    onConnectionEvent?: (event: PeerConnectionEvent) => void;
}, callback: (payload: RequestPayload) => Promise<FfiResponseHead>) => Promise<void>;
export type RawFetchFn = (endpointHandle: number, nodeId: string, url: string, method: string, headers: [string, string][], reqBodyHandle: bigint | null, fetchToken: bigint, directAddrs: string[] | null) => Promise<FfiResponse>;
export type AllocBodyWriterFn = () => bigint | Promise<bigint>;
export type RawConnectFn = (endpointHandle: number, nodeId: string, path: string, headers: [string, string][]) => Promise<FfiDuplexStream>;
export declare abstract class IrohAdapter {
    ...
    abstract rawFetch(endpointHandle: number, nodeId: string, url: string, method: string, headers: [string, string][], reqBodyHandle: bigint | null, fetchToken: bigint, directAddrs: string[] | null): Promise<FfiResponse>;
    abstract rawServe(endpointHandle: number, options: {
        onConnectionEvent?: (event: PeerConnectionEvent) => void;
    }, callback: (payload: RequestPayload) => Promise<FfiResponseHead>): Promise<void>;
    ...
    rawConnect(_endpointHandle: number, _nodeId: string, _path: string, _headers: [string, string][]): Promise<FfiDuplexStream>;
