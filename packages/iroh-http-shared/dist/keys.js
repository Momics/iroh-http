...
exports.SecretKey = exports.PublicKey = void 0;
...
class PublicKey {
    ...
    toString() {
        if (this.#str === null)
            this.#str = base32Encode(this.#bytes);
        return this.#str;
    }
    ...
    async verify(data, signature) {
        try {
            const key = await crypto.subtle.importKey("raw", this.#bytes, ED25519, false, ["verify"]);
...
class SecretKey {
    ...
    get publicKey() {
        if (this.#publicKey === null) {
            throw new TypeError("publicKey not yet available — call await secretKey.derivePublicKey() first");
        }
     ...
     * @internal
     */
    static _fromBytesWithPublicKey(bytes, publicKey) {
    ...
    async derivePublicKey() {
        ...
        const cryptoKey = await crypto.subtle.importKey("pkcs8", ed25519Pkcs8(this.#bytes), ED25519, true, ["sign"]);
     ...
     */
    async sign(data) {
        const cryptoKey = await crypto.subtle.importKey("pkcs8", ed25519Pkcs8(this.#bytes), ED25519, false, ["sign"]);
