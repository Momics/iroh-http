...
function makeFetch(adapter, endpointHandle) {
    return async function irohFetch(peerOrInput, inputOrInit, maybeInit) {
        ...
        if (typeof inputOrInit === "string" || inputOrInit instanceof URL) {
            ...
            nodeId = (0, keys_js_1.resolveNodeId)(peerOrInput);
            ...
            if (!/^httpi:\/\//i.test(raw)) {
                throw new TypeError(`iroh-http fetch() requires either an httpi:// URL or (peer, path) arguments. ` +
                    `Got: "${raw.slice(0, 80)}"`);
            }
        ...
        if (/^https?:\/\//i.test(url)) {
            ...
            throw new TypeError(`iroh-http URLs must use the "httpi://" scheme, not "${scheme}". ` +
        ...
        if ((method.toUpperCase() === "GET" || method.toUpperCase() === "HEAD") &&
            init?.body != null) {
            throw new TypeError(`Request body is not allowed for ${method} requests. ` +
        ...
        if (bodyPipePromise) {
            bodyPipePromise.catch((err) => console.error("[iroh-http] request body pipe error:", err));
        }
